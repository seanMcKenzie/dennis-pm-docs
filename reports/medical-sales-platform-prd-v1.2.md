...
**Version:** 1.2  
**Status:** Updated — Open Questions OQ-1 through OQ-4 Resolved (Mar 2, 2026)
...
| 1.2 | Mar 2, 2026 | Dennis Reynolds | OQ-1 through OQ-4 resolved: SaaS multi-tenant, React Native iOS-first, AWS cloud-hosted, standalone CRM v1; all downstream sections updated accordingly |
...
**Platform:** Mobile-first — React Native (iOS 16+ primary; Android Phase 2) with a web companion for managers and reporting.

**Deployment Model:** SaaS — multi-tenant, cloud-hosted on AWS. Each subscribing organization receives isolated data access. No self-hosted option in v1.

**CRM Strategy:** Standalone CRM for v1. No external CRM integrations (Salesforce, Veeva, etc.) in Phase 1. CSV export in Phase 2; API integrations in Phase 3.
...
**FR-048** — The system shall support data export of CRM activity logs (calls, tasks, opportunities, orders) in CSV format. *(Phase 2 feature — OQ-4 confirmed: v1 is standalone CRM with no live external integrations. CSV export provides a manual data bridge to Salesforce, Veeva, or other external systems for organizations that need it. Bidirectional API integrations are Phase 3 scope.)*
...
**NFR-019** — *(Confirmed v1.2 — SaaS multi-tenant)* The system shall enforce organization-level data isolation in a multi-tenant SaaS architecture. One organization's CRM data shall never be accessible to another organization's users. All CRM tables must include `org_id` and all API queries must filter by the authenticated user's `org_id`. Physician/provider data (sourced from public CMS datasets) is shared read-only infrastructure across all tenants. AWS deployment on RDS PostgreSQL; row-level security policies are the enforcement mechanism. See Section 5.0 for full tenancy decision.
...
**NFR-025** — *(Updated v1.2 — OQ-2 resolved)* The mobile application shall be built in **React Native**. Phase 1 targets **iOS 16+** exclusively. Android support (minimum Android 12) is a Phase 2 deliverable. The React Native codebase must be structured to support Android in a follow-on phase without a full rewrite.
...
**NFR-029** — The mobile app shall pass **Apple App Store** review requirements without policy exceptions. Phase 1 (iOS only). Google Play Store compliance applies in Phase 2 when Android support is introduced.
...
### 5.0 Architecture & Technology Decisions (Locked — v1.2)

The following decisions were confirmed by Sean (March 2, 2026) resolving OQ-1 through OQ-4. These are final for v1.

| Decision | Choice | Notes |
|---|---|---|
| **Tenancy model** | SaaS — multi-tenant | All CRM tables require `org_id` column for org isolation. Physician/provider data is shared across tenants (read-only public CMS data). |
| **Mobile framework** | React Native | iOS 16+ in Phase 1. Android follows in a later phase. Apple Maps deep-links (FR-NEW-004) are iOS-only as specified. |
| **Cloud hosting** | AWS | ECS or EKS for container orchestration; RDS PostgreSQL + PostGIS for primary database. No self-hosted or on-premises option in v1. |
| **CRM integration** | Standalone v1 | No Salesforce, Veeva, or HubSpot integration in Phase 1. CSV export (FR-048) is Phase 2. Bidirectional CRM API integration is Phase 3. |

**org_id isolation requirement:** Every CRM table (`CRMCall`, `CRMTask`, `CRMOpportunity`, `CRMOrder`, `CRMSample`, and any future CRM table) must include an `org_id UUID NOT NULL` column with a foreign key to the `Organization` table. Row-level access control must filter all CRM queries by `org_id`. This is non-negotiable for multi-tenant data isolation. See NFR-019.
...
**RISK-NEW-003 — Multi-Tenant Data Isolation Breach** *(Added v1.2 — OQ-1 confirmed SaaS)*
Confirmed SaaS architecture means a misconfigured query or missing `org_id` filter could expose one customer's CRM data to another. This is a trust-destroying incident that cannot be patched post-launch. Mitigation: (1) Row-level security enforced at the database layer (PostgreSQL RLS policies), not only at the application layer. (2) All CRM tables must have `org_id NOT NULL` — enforced in schema migrations, not optional. (3) Integration tests must include explicit cross-tenant isolation checks (attempt to access Org A data while authenticated as Org B; must return 0 results and 403). (4) Code review checklist must include org_id filter verification for every new CRM query.

**RISK-NEW-004 — SaaS Billing & Org Lifecycle Complexity** *(Added v1.2 — OQ-1 confirmed SaaS)*
SaaS requires subscription management, tenant provisioning, org-level admin, and offboarding/data-deletion workflows that an internal tool does not need. If not planned for in the data model from day one, adding these later is expensive. Mitigation: Sprint 2 must include Organization and subscription lifecycle table design. Data deletion/export for churned customers must be in Phase 2 scope.
...
### ✅ RESOLVED (v1.2 — March 2, 2026)

**OQ-1 — SaaS vs. Internal Tool** ✅ RESOLVED  
**Decision:** SaaS — multi-tenant architecture. This platform is a SaaS product sold to multiple pharma/device companies, not an internal tool for one org. Multi-tenancy is required from day one. All CRM tables must include `org_id` for org isolation. See NFR-019 and Section 5.0.

**OQ-2 — Mobile Platform** ✅ RESOLVED  
**Decision:** React Native, iOS-first. Phase 1 targets iOS 16+. Android follows in a later phase. See NFR-025. The developer does not need to re-debate this. React Native. iOS. Done.

**OQ-3 — Cloud Hosting** ✅ RESOLVED  
**Decision:** Cloud-hosted on AWS. No self-hosted or on-premises option in v1. Infrastructure: ECS or EKS for containers, RDS PostgreSQL + PostGIS for the primary database. This is the correct choice for a data-heavy SaaS platform with 8GB+ ingestion jobs. See Section 5.0.

**OQ-4 — CRM Integration** ✅ RESOLVED  
**Decision:** Standalone CRM for v1. No Salesforce, Veeva, or HubSpot integration in Phase 1. We are the CRM. CSV export (FR-048) is Phase 2. Bidirectional API integrations are Phase 3. Organizations coming from an existing CRM will migrate manually.

---

### 🔓 OPEN — Pending Answers from Sean

**OQ-5 — Commercial data supplement:** The free CMS data is 18–24 months stale for Part B/D. Is there budget to add a commercial data layer (IQVIA, Symphony, or Definitive Healthcare) for more current prescribing data? This significantly changes the cost structure.

**OQ-6 — Email integration:** Do reps want emails they send to physicians auto-logged in the CRM? This requires integration with Gmail/Outlook and adds significant scope.

**OQ-7 — Mapping provider:** ~~Google Maps vs. Mapbox vs. Apple Maps?~~ *(Resolved in v1.1 — no routing API required for v1. Geocoding still uses Google Maps or Mapbox; see RISK-003.)*

**OQ-8 — Sample management compliance:** Some regulated sample distribution requires FDA-compliant e-signature and chain-of-custody tracking. Is pharmaceutical sample management in scope, or just internal tracking?

**OQ-9 — International scope:** US only for now, or will international markets (Canada, EU) be required? This changes data sourcing completely.

**OQ-10 — AI prompt export admin toggle:** *(New — v1.1)* Should organizations be able to disable the AI prompt export feature for reps on networks with strict data handling policies? If yes, this requires an admin configuration setting.
